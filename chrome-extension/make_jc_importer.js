...
function generateTsv(metadata, templateText) {
  ...
  const cols = buildTsvColumnDefs(prefix, metadata, customTemplate);
  if (!cols.length) return null;

  // ItemType行: カスタムテンプレートがあればその row0、なければデフォルト
  const tmplRow0 = effectiveTemplate[0];
  const n = cols.length;
  const row1 = [tmplRow0[0] || '#ItemType', tmplRow0[1] || '', tmplRow0[2] || '', ...Array(Math.max(0, n - 3)).fill('')];
