...
// TSV_HEADERS_TEMPLATE からフィールドグループを構築。
...
function groupTsvColumns(prefix) {
  const DEFAULT_PREFIX = 'item_30002_';
  const [, keyRow, labelRow, sysRow, conRow] = TSV_HEADERS_TEMPLATE;
  const groups = [];
  let cur = null;
  ...
  for (let i = 0; i < keyRow.length; i++) {
    ...
    if (isTsvExcluded(baseKey)) continue;

    const outKey = (prefix !== DEFAULT_PREFIX)
      ? baseKey.replace(DEFAULT_PREFIX, prefix)
      : baseKey;
    const lookupKey = baseKey; // always item_30002_ for metadata access
...
function buildTsvColumnDefs(prefix, metadata) {
  const groups = groupTsvColumns(prefix);
  const columns = [];
...
function generateTsv(metadata, templateText) {
  ...
  const tmpl = TSV_HEADERS_TEMPLATE[0]; // ['#ItemType', name, schema]
  const n = cols.length;
