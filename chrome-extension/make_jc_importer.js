...
// ===== DataCite resourceTypeGeneral → JPCOAR 資源タイプ個別マッピング（docs/datacite_jpcoar_mapping.md §6） =====
// 正規化（PascalCase→大文字境界分割→小文字化）のみで TITLE_MAPS.resourcetype に一致する14値はここに含めない。
const DATACITE_RESOURCE_TYPE_MAP = {
  'Audiovisual':          'video',
  'Award':                'other',
  'BookChapter':          'book part',
  'Collection':           'other',
  ...
  'Service':              'other',
  'Standard':             'other',
  'StudyRegistration':    'research protocol',
  'Text':                 'other',
};

// resourceTypeGeneral → JPCOAR 資源タイプ（正規化→個別マップの順。未知値は空文字フォールバック）
function normalizeDataCiteResourceType(resourceTypeGeneral) {
  const raw = resourceTypeGeneral || '';
  if (!raw) return '';
  const normalized = raw.replace(/([a-z0-9])([A-Z])/g, '$1 $2').toLowerCase();
  if (TITLE_MAPS.resourcetype.includes(normalized)) return normalized;
...
const DATACITE_RELATION_TYPE_MAP = {
  ...
  'IsVariantFormOf':    'isFormatOf',
  'IsOriginalFormOf':   'hasFormat',
  'Obsoletes':          'replaces',
  'IsObsoletedBy':      'isReplacedBy',
  'IsPublishedIn':      'isPartOf',
};

// ===== DataCite relatedIdentifierType → ツール識別子タイプマッピング（docs/datacite_jpcoar_mapping.md §7-2） =====
// 未収載13値（bibcode等）は http(s):// で始まる値のみ URI として登録（コード側で個別判定）
const DATACITE_RELATED_ID_TYPE_MAP = {
  'ARK':    'ARK',
  'arXiv':  'arXiv',
  'DOI':    'DOI',
  'EISSN':  'EISSN',
...
// ===== DataCite funderIdentifierType → ツール助成機関識別子タイプマッピング（docs/datacite_jpcoar_mapping.md §11） =====
const DATACITE_FUNDER_ID_TYPE_MAP = {
  'Crossref Funder ID': 'Crossref Funder',
  'ROR':                'ROR',
  'ISNI':               'ISNI',
  'GRID':               'Other',
  'Other':              'Other',
...
// ===== KAKENHI プログラム情報定数（JPCOAR 2.0 fundingStream）=====
...
// ===== 助成機関識別子タイプ→URI マッピング（JPCOAR 2.0） =====
const FUNDER_ID_TYPE_URI_MAP = {
...
const JPCOAR_SUBFIELD_LINKS = {
  ...
  // 作成者所属 下位項目 (#3-.6-.x)
  creator_aff_id:           'https://schema.irdb.nii.ac.jp/ja/schema/2.0/3-.6-.1',
  creator_aff_name:         'https://schema.irdb.nii.ac.jp/ja/schema/2.0/3-.6-.2',
  // 寄与者所属 下位項目 (#4-.6-.x)
  contributor_aff_id:       'https://schema.irdb.nii.ac.jp/ja/schema/2.0/4-.6-.1',
  contributor_aff_name:     'https://schema.irdb.nii.ac.jp/ja/schema/2.0/4-.6-.2',
  // 権利者情報 (#7)
  rightsholder_id:          'https://schema.irdb.nii.ac.jp/ja/schema/2.0/7-.1',
  rightsholder_name:        'https://schema.irdb.nii.ac.jp/ja/schema/2.0/7-.2',
  // 関連情報 (#20)
...
const TITLE_MAPS = {
  ...
  // 査読の有無（OA Assist が SWORD API 経由で設定）
  subitem_peer_reviewed: ['','Peer reviewed','Not peer reviewed'],
  // 内容記述タイプ
...
// ===== 3.2c DataCite REST API =====
// DataCite API は CORS 対応・認証不要のため、標準版・Chrome拡張版とも extensionFetch() を使わず直接 fetch する。
// publisher/affiliation はクエリパラメータを付けないとオブジェクト形（識別子付き）で返らないため必須（docs/datacite_jpcoar_mapping.md §14-1）。
async function fetchDataCite(doi) {
  const resp = await fetch(`https://api.datacite.org/dois/${encodeURIComponent(doi)}?publisher=true&affiliation=true`, {
...
async function fetchJgn(awardNumber) {
  ...
  try {
    ...
    // プログラム情報（scheme）: JPCOAR 2.0 fundingStream
...
async function fetchCrossrefData(doi) {
  ...
  currentBatchIndex = allMetadata.length - 1;
  updateBatchPanel();
}
...
// ===== 3.6b DataCite取得フロー =====
// DataCite API は CORS 対応・認証不要のため、標準版・拡張版とも同一経路で取得できる（JaLCのような環境分岐は不要）。
...
// ===== 3.7 バッチ蓄積ヘルパー =====
// 現在のDOMの編集内容をバッチに保存（表示中のメタデータがある場合）
function saveCurrent() {
  if (currentBatchIndex < 0 || currentBatchIndex >= allMetadata.length) return;
  if (document.getElementById('preview-area')?.style.display === 'none') return;
...
async function fetchAndAccumulate(doi) {
  ...
  if (ra === 'DataCite') {
    // DataCite は CORS 対応・認証不要のため、標準版・拡張版とも環境分岐なしで取得できる
    await fetchDataCiteData(doi);
    return { status: 'ok' };
  }
...
function buildDataCiteAuthors(creators) {
  return (creators || []).map(cr => {
    ...
    // 研究者ID: ORCID は URL形/bare形が混在するため URL形に正規化。e-Rad_Researcher・NRID はそのまま対応（ツールselectと一致）
    const nameIdentifiers = (cr.nameIdentifiers || []).map(ni => {
      const scheme = ni.nameIdentifierScheme || '';
      const idVal  = ni.nameIdentifier || '';
      if (!idVal) return null;
      if (scheme === 'ORCID') {
        const bareId = idVal.replace(/^https?:\/\/orcid\.org\//i, '');
        return { nameIdentifier: bareId, nameIdentifierScheme: 'ORCID', nameIdentifierURI: `https://orcid.org/${bareId}` };
      }
      if (scheme === 'e-Rad_Researcher' || scheme === 'NRID') {
        return { nameIdentifier: idVal, nameIdentifierScheme: scheme, nameIdentifierURI: '' };
      }
      return null;
    }).filter(Boolean);

    // 所属: affiliationIdentifierScheme は ROR/ISNI/Ringgold/GRID の4種（?affiliation=true 前提）
    const affiliations = (cr.affiliation || []).map(aff => {
      const name = typeof aff === 'string' ? aff : (aff.name || '');
      const affiliationNames = name ? [{ affiliationName: name, affiliationNameLang: '' }] : [];

      const affiliationNameIdentifiers = [];
      const idType = (typeof aff === 'object' && aff.affiliationIdentifierScheme) || '';
      const idVal  = (typeof aff === 'object' && aff.affiliationIdentifier) || '';
      if (idVal) {
        if (idType === 'ROR') {
          const rorId = idVal.replace(/^https?:\/\/ror\.org\//i, '').replace(/\/$/, '');
          affiliationNameIdentifiers.push({
            affiliationNameIdentifier: rorId, affiliationNameIdentifierScheme: 'ROR',
            affiliationNameIdentifierURI: `https://ror.org/${rorId}`,
          });
        } else if (idType === 'ISNI') {
          affiliationNameIdentifiers.push({
            affiliationNameIdentifier: idVal, affiliationNameIdentifierScheme: 'ISNI',
            affiliationNameIdentifierURI: `https://isni.org/isni/${idVal}`,
          });
        } else if (idType === 'Ringgold' || idType === 'GRID') {
          affiliationNameIdentifiers.push({
            affiliationNameIdentifier: idVal, affiliationNameIdentifierScheme: idType,
            affiliationNameIdentifierURI: '',
          });
        }
      }
    ...
    const buildEntry = async (awardNum) => {
      ...
      if (funderDoi) {
        obj.subitem_funder_identifiers = {
          ...
          subitem_funder_identifier_type_uri: FUNDER_ID_TYPE_URI_MAP['Crossref Funder'] || '',
        };
      } else {
        obj.subitem_funder_identifiers = { subitem_funder_identifier: '', subitem_funder_identifier_type: '', subitem_funder_identifier_type_uri: '' };
      }
      ...
      // プログラム情報（JPCOAR 2.0 fundingStream）
      if (kakenResult?.fundingStreams?.length) {
        // JGN 結果: scheme から取得
        obj.subitem_funding_streams = kakenResult.fundingStreams.map(fs => ({
          subitem_funding_stream:          fs.fundingStream,
          subitem_funding_stream_language:  fs.fundingStreamLang,
        }));
        obj.subitem_funding_stream_identifiers = {
          subitem_funding_stream_identifier:          kakenResult.fundingStreamId || '',
          subitem_funding_stream_identifier_type:     kakenResult.fundingStreamId ? 'JGN_fundingStream' : '',
          subitem_funding_stream_identifier_type_uri: '',
        };
      } else if (kakenResult && looksKakenhi) {
        // KAKEN XML / CiNii 結果: KAKENHI 固定値
        obj.subitem_funding_streams = KAKENHI_FUNDING_STREAM.map(fs => ({
          subitem_funding_stream:          fs.fundingStream,
          subitem_funding_stream_language:  fs.fundingStreamLang,
        }));
        obj.subitem_funding_stream_identifiers = {
          subitem_funding_stream_identifier: '', subitem_funding_stream_identifier_type: '', subitem_funding_stream_identifier_type_uri: '',
        };
      } else {
        obj.subitem_funding_streams = [];
        obj.subitem_funding_stream_identifiers = {
          subitem_funding_stream_identifier: '', subitem_funding_stream_identifier_type: '', subitem_funding_stream_identifier_type_uri: '',
        };
      }
...
async function buildJaLCFunders(jalcFundList) {
  const entries = await Promise.all((jalcFundList || []).map(async (f) => {
    ...
    const funderIdentifiers = funderDoi
      ? { subitem_funder_identifier: `https://doi.org/${funderDoi}`, subitem_funder_identifier_type: 'Crossref Funder', subitem_funder_identifier_type_uri: FUNDER_ID_TYPE_URI_MAP['Crossref Funder'] || '' }
      : { subitem_funder_identifier: '', subitem_funder_identifier_type: '', subitem_funder_identifier_type_uri: '' };
...
// ===== 4.4c DataCite 助成情報マッピング（docs/datacite_jpcoar_mapping.md §11） =====
// buildJaLCFunders とは独立した関数として実装する（JaLCパスへの影響を避けるため共通化しない）。
// DataCite の fundingReferences[] は award_number_group_list のようなグルーピングを持たず、
// エントリごとに awardNumber が最大1件（無ければ空文字1件）である前提で1エントリ=1件を返す。
async function buildDataCiteFunders(fundingReferences) {
  return Promise.all((fundingReferences || []).map(async (f) => {
    const funderName  = f.funderName || '';
    const funderNames = funderName ? [{ subitem_funder_name: funderName, subitem_funder_name_language: 'en' }] : [];

    // 助成機関識別子: Crossref Funder ID は既存パス同様 DOI形へ変換。それ以外は値をそのまま設定
    const idType = f.funderIdentifierType || '';
    const idVal  = f.funderIdentifier || '';
    let funderDoi = '';
    let funderIdentifiers;
    if (idType === 'Crossref Funder ID' && idVal) {
      funderDoi = idVal.replace(/^https?:\/\/(dx\.)?doi\.org\//i, '');
      funderIdentifiers = {
        subitem_funder_identifier:          `https://doi.org/${funderDoi}`,
        subitem_funder_identifier_type:     'Crossref Funder',
        subitem_funder_identifier_type_uri: FUNDER_ID_TYPE_URI_MAP['Crossref Funder'] || '',
      ...
      const mappedType = DATACITE_FUNDER_ID_TYPE_MAP[idType] || '';
      funderIdentifiers = {
        subitem_funder_identifier:          idVal,
        subitem_funder_identifier_type:     mappedType,
        subitem_funder_identifier_type_uri: FUNDER_ID_TYPE_URI_MAP[mappedType] || '',
      };
    } else {
      funderIdentifiers = { subitem_funder_identifier: '', subitem_funder_identifier_type: '', subitem_funder_identifier_type_uri: '' };
    }
    const isJsps = funderDoi === JSPS_FUNDER_DOI;

    const awardNum = f.awardNumber || '';
    const obj = { subitem_funder_names: funderNames, subitem_funder_identifiers: funderIdentifiers };
    ...
    // 研究課題名: KAKEN/JGN結果を優先。無ければ DataCite の awardTitle を使用（Crossrefパスに無い利点、§11）
    obj.subitem_award_titles = kakenResult?.titles?.length
      ? kakenResult.titles
      : (f.awardTitle ? [{ subitem_award_title: f.awardTitle, subitem_award_title_language: 'en' }] : []);
...
// ================================================================
// STEP 4b: DataCite マッピング（docs/datacite_jpcoar_mapping.md 準拠）
// ================================================================

// ===== 4.5a 日付値の正規化（§8。日付フィールドへ登録する値のみ対象。Coverageは対象外） =====
// タイムスタンプ（2022-12-06T18:46:04Z）→ 日付部分のみ、範囲（RKMS-ISO8601 start/end）→ 開始側（無ければ終了側）
function normalizeDataCiteDate(dateStr) {
  let v = (dateStr || '').trim();
  if (!v) return '';
  if (v.includes('/')) {
    const [start, end] = v.split('/');
    v = (start && start.trim()) ? start.trim() : (end || '').trim();
  }
  const isoMatch = v.match(/^(\d{4}-\d{2}-\d{2})T/);
  return isoMatch ? isoMatch[1] : v;
}

// ===== 4.5b 日付・時間的範囲マッピング（§8） =====
// dateType がツールの日付タイプ9値に含まれるものは日付として登録、Coverageは時間的範囲へ分離、
// Withdrawn/Otherは破棄。datesが空またはIssuedが無い場合はpublicationYearからIssuedを生成する。
function buildDataCiteDatesAndTemporal(dates, publicationYear) {
  const dateEntries = [];
  ...
  (dates || []).forEach(d => {
    ...
    if (!dt || !tp) return;
    if (tp === 'Coverage') {
      temporalEntries.push({ subitem_temporal_text: dt, subitem_temporal_language: '' });
      return;
    }
    if (!TITLE_MAPS.subitem_date_issued_type.includes(tp)) return; // Withdrawn/Other 等は破棄
    const normalized = normalizeDataCiteDate(dt);
    if (normalized) dateEntries.push({ subitem_date_issued_datetime: normalized, subitem_date_issued_type: tp });
  });
  if (!dateEntries.some(d => d.subitem_date_issued_type === 'Issued') && publicationYear) {
    dateEntries.push({ subitem_date_issued_datetime: String(publicationYear), subitem_date_issued_type: 'Issued' });
  }
  return { dateEntries, temporalEntries };
}

// ===== 4.5c 言語マッピング（§10。本文言語は639-2/T、サブフィールド言語は639-1系で2系統を区別） =====
// 本文言語: ISO 639-1 → 639-2/T の最小限テーブル（テストDOIで確認済みの言語を含む）。未収載は空（既定'eng'にはしない）
const DATACITE_LANG_1_TO_2T = {
  'ja': 'jpn', 'en': 'eng', 'fr': 'fra', 'de': 'deu', 'zh': 'zho',
  'ko': 'kor', 'es': 'spa', 'pt': 'por', 'ru': 'rus', 'it': 'ita',
};
// ISO 639-2/B → 639-2/T（3文字コードがB表記の場合のみ変換。相違があるのはこの20言語のみ）
const DATACITE_LANG_2B_TO_2T = {
...
function mapDataCiteLanguageToSubitemLanguage(language) {
  const primary = (language || '').split('-')[0].toLowerCase();
  if (!primary) return '';
  let candidate = '';
  if (primary.length === 2) candidate = DATACITE_LANG_1_TO_2T[primary] || '';
  else if (primary.length === 3) candidate = DATACITE_LANG_2B_TO_2T[primary] || primary;
  return TITLE_MAPS.subitem_language.includes(candidate) ? candidate : '';
}

// サブフィールド言語（titles[].lang 等）: TITLE_MAPS.language（639-1系）に含まれる場合のみそのまま採用
function mapDataCiteSubfieldLanguage(lang) {
  return (lang && TITLE_MAPS.language.includes(lang)) ? lang : '';
}

// ===== 4.5d 関連情報マッピング（§7。relationType/relatedIdentifierType とも未対応値は当該エントリを破棄） =====
function mapDataCiteRelatedIdentifiers(relatedIdentifiers) {
  const relations = [];
  (relatedIdentifiers || []).forEach(r => {
    const relType = DATACITE_RELATION_TYPE_MAP[r.relationType || ''];
    if (!relType) return;

    const id = r.relatedIdentifier || '';
    if (!id) return;
    let idType = DATACITE_RELATED_ID_TYPE_MAP[r.relatedIdentifierType || ''];
    if (!idType) {
      // 未収載13値（bibcode等）: http(s)://で始まる場合のみURIとして登録、それ以外は破棄
      if (/^https?:\/\//i.test(id)) idType = 'URI';
      else return;
    }

    const idText = idType === 'DOI' && !/^https?:\/\//i.test(id) ? `https://doi.org/${id}` : id;
    relations.push({
      subitem_relation_type: relType,
      subitem_relation_type_id: { subitem_relation_type_id_text: idText, subitem_relation_type_select: idType },
      subitem_relation_name: [],
    });
  });
  return relations;
...
function extractDataCiteBibliographicInfo(relatedItems, container) {
  ...
  const sourceTitleText = journalItem.titles?.title || '';
  ...
  return {
    sourceIdentifiers,
    sourceTitles,
    volume:    journalItem.volume    || cont.volume    || '',
    issue:     journalItem.issue     || cont.issue     || '',
    firstPage: journalItem.firstPage || cont.firstPage || '',
    lastPage:  journalItem.lastPage  || cont.lastPage  || '',
  };
}

// ===== 4.5f 位置情報マッピング（§13。point/box/placeのみ対象、polygonはツールUI非対応のため対象外） =====
function buildDataCiteGeolocations(geoLocations) {
  return (geoLocations || []).map(geo => {
    const point = geo.geoLocationPoint || {};
    const box   = geo.geoLocationBox   || {};
    const place = geo.geoLocationPlace || '';
    return {
      subitem_geolocation_point: {
        subitem_point_longitude: point.pointLongitude != null ? String(point.pointLongitude) : '',
        subitem_point_latitude:  point.pointLatitude  != null ? String(point.pointLatitude)  : '',
      },
      subitem_geolocation_box: {
        subitem_west_longitude: box.westBoundLongitude != null ? String(box.westBoundLongitude) : '',
        subitem_east_longitude: box.eastBoundLongitude != null ? String(box.eastBoundLongitude) : '',
        subitem_south_latitude: box.southBoundLatitude != null ? String(box.southBoundLatitude) : '',
        subitem_north_latitude: box.northBoundLatitude != null ? String(box.northBoundLatitude) : '',
      },
      subitem_geolocation_place: place ? [{ subitem_geolocation_place_text: place }] : [],
    };
  }).filter(g =>
    g.subitem_geolocation_point.subitem_point_longitude || g.subitem_geolocation_point.subitem_point_latitude ||
    g.subitem_geolocation_box.subitem_west_longitude || g.subitem_geolocation_box.subitem_east_longitude ||
    g.subitem_geolocation_place.length
  );
}
...
async function mapToItemTypeDataCite(attrs, oaJson) {
  ...
  // ===== タイトル・その他のタイトル =====
  const allTitles = attrs.titles || [];
  const titles = allTitles.filter(t => !t.titleType).map(t => {
    const rawLang = t.lang || '';
    if (!rawLang) return { subitem_title: t.title || '', subitem_title_language: 'en', _warnLang: true };
    return { subitem_title: t.title || '', subitem_title_language: mapDataCiteSubfieldLanguage(rawLang) };
  }).filter(t => t.subitem_title);

  const ALT_TITLE_TYPES = ['AlternativeTitle', 'Subtitle', 'TranslatedTitle', 'Other'];
  const alternativeTitles = allTitles.filter(t => ALT_TITLE_TYPES.includes(t.titleType)).map(t => ({
    subitem_alternative_title: t.title || '',
    subitem_alternative_title_language: mapDataCiteSubfieldLanguage(t.lang || ''),
  })).filter(t => t.subitem_alternative_title);

  // ===== 作成者 =====
  const creators = buildDataCiteAuthors(attrs.creators);

  // ===== 権利情報 =====
  // info:eu-repo/semantics/* は権利情報でなくアクセス権の示唆（OA表明）のため取り込まない（§9）
  const rights = (attrs.rightsList || [])
    .filter(r => !/^info:eu-repo\/semantics\//i.test(r.rightsUri || ''))
    .map(r => ({
      subitem_rights: r.rights || '',
      subitem_rights_language: '',
      subitem_rights_resource: r.rightsUri || '',
    }));

  // ===== 主題 =====
  const subjects = (attrs.subjects || []).map(s => ({
    subitem_subject: s.subject || '',
    subitem_subject_language: mapDataCiteSubfieldLanguage(s.lang || ''),
    subitem_subject_scheme: 'Other',
    subitem_subject_uri: '',
  })).filter(s => s.subitem_subject);

  // ===== 資源タイプ =====
  const resourcetype = normalizeDataCiteResourceType(attrs.types?.resourceTypeGeneral);
  const resourceuri  = resourcetype ? (RESOURCE_TYPE_MAP[resourcetype] || '') : '';

  // ===== 内容記述 =====
  const DESC_TYPES = ['Abstract', 'Methods', 'TableOfContents', 'TechnicalInfo', 'Other'];
  const descriptions = (attrs.descriptions || [])
    .filter(d => DESC_TYPES.includes(d.descriptionType))
    .map(d => ({
      subitem_description: d.description || '',
      subitem_description_language: mapDataCiteSubfieldLanguage(d.lang || ''),
      subitem_description_type: d.descriptionType,
    }))
    .filter(d => d.subitem_description);

  // ===== 言語 =====
  const lang639_2 = mapDataCiteLanguageToSubitemLanguage(attrs.language || '');

  // ===== 出版者 =====
  const publisherObj  = attrs.publisher || {};
  const publisherName = typeof publisherObj === 'string' ? publisherObj : (publisherObj.name || '');
  const publisherLang = (typeof publisherObj === 'object' && publisherObj.lang) || 'en';
  const publishers = publisherName ? [{ subitem_publisher: publisherName, subitem_publisher_language: publisherLang }] : [];

  // ===== 日付・時間的範囲 =====
  const { dateEntries, temporalEntries } = buildDataCiteDatesAndTemporal(attrs.dates, attrs.publicationYear);
  const issuedDate = dateEntries.find(d => d.subitem_date_issued_type === 'Issued')?.subitem_date_issued_datetime || '';

  // ===== 書誌情報（収録物） =====
  const biblio = extractDataCiteBibliographicInfo(attrs.relatedItems, attrs.container);
  const sourceIdentifiers = biblio.sourceIdentifiers;
  const sourceTitles      = biblio.sourceTitles;

  // ===== NCID（ISSNが取れた場合のみ、JaLCパスと同様に既存fetchNcid()を再利用） =====
  const allIssns = sourceIdentifiers.map(si => si.subitem_source_identifier);
  if (allIssns.length) {
    const ncid = await fetchNcid(allIssns);
    if (ncid) {
  ...
  relations.push(...mapDataCiteRelatedIdentifiers(attrs.relatedIdentifiers));
  ...
  const metadata = {
    ...
    item_30002_title0: titles,
    item_30002_alternative_title1: alternativeTitles,
    item_30002_creator2: creators,
    item_30002_contributor3: [],
    ...
    item_30002_identifier_registration17: { subitem_identifier_reg_text: '', subitem_identifier_reg_type: '' },
    item_30002_relation18: relations,
    item_30002_temporal19: temporalEntries,
    item_30002_geolocation20: geolocations,
    item_30002_funding_reference21: await buildDataCiteFunders(attrs.fundingReferences),
    item_30002_source_identifier22: sourceIdentifiers,
    item_30002_source_title23: sourceTitles,
    item_30002_volume_number24: { subitem_volume: biblio.volume },
    item_30002_issue_number25: { subitem_issue: biblio.issue },
    item_30002_number_of_pages26: { subitem_number_of_pages: '' },
    item_30002_page_start27: { subitem_start_page: biblio.firstPage },
    item_30002_page_end28: { subitem_end_page: biblio.lastPage },

    item_30002_bibliographic_information29: sourceTitles.length ? {
      bibliographicIssueDates: issuedDate
        ? { bibliographicIssueDate: issuedDate, bibliographicIssueDateType: 'Issued' }
        : {},
      bibliographicIssueNumber:   biblio.issue,
      bibliographicVolumeNumber:  biblio.volume,
      bibliographicPageStart:     biblio.firstPage,
      bibliographicPageEnd:       biblio.lastPage,
      bibliographicNumberOfPages: '',
      bibliographic_titles: sourceTitles.map(st => ({
        bibliographic_title:     st.subitem_source_title,
        bibliographic_titleLang: st.subitem_source_title_language,
...
const FIELD_DEFS = [
  ...
  { key: 'item_30002_version_type15', label: '出版タイプ', type: 'object',
    fields: [
      ...
      { k: 'subitem_peer_reviewed', l: '査読の有無', t: 'select', o: 'subitem_peer_reviewed' },
...
function renderOnePerson(person, idx, keys) {
  ...
  const { wrapper: affWrap, content: affCont } = createNestedSectionHeader(affLabel, 2, (cont) => {
    ...
    cont.appendChild(renderOneAffiliation(emptyAff, ai, keys, sfx));
  }, JPCOAR_SUBFIELD_LINKS[sfx + '_affiliation']);
  (person[affKey] || []).forEach((aff, ai) => {
    affCont.appendChild(renderOneAffiliation(aff, ai, keys, sfx));
  });
...
// ----- 所属機関: 1件分のDOM生成 -----
function renderOneAffiliation(aff, ai, keys, sfx) {
  const {
  ...
  }, sfx ? JPCOAR_SUBFIELD_LINKS[sfx + '_aff_id'] : undefined);
...
function renderOneFunder(funder, idx, defLabel) {
  ...
  // (2) タイプ行（変更時に updateFunderLookup 呼び出し + URI 自動設定）
  ...
  fundContent.appendChild(createFieldRow('識別子タイプ', fId.subitem_funder_identifier_type || '', 'select',
    'subitem_funder_identifier_type', {
      ...
      onChange: () => {
        ...
        const uriEl  = fundContent.querySelector('[data-field-key="subitem_funder_identifier_type_uri"] input');
        if (typeEl && uriEl) uriEl.value = FUNDER_ID_TYPE_URI_MAP[typeEl.value] || '';
      },
    }));
  // (2b) 識別子タイプURI（読み取り専用、タイプ選択時に自動設定）
  fundContent.appendChild(createFieldRow('識別子タイプURI', fId.subitem_funder_identifier_type_uri || '', 'text', null,
    { fieldKey: 'subitem_funder_identifier_type_uri' }));
  ...
  awardLookupBtn.onclick = async (e) => {
    ...
    try {
      ...
      const typeUriInput = fundContent.querySelector('[data-field-key="subitem_funder_identifier_type_uri"] input');
      if (idInput) idInput.value = `https://doi.org/${doi}`;
      if (typeSelect) typeSelect.value = 'Crossref Funder';
      if (typeUriInput) typeUriInput.value = FUNDER_ID_TYPE_URI_MAP['Crossref Funder'] || '';
      updateFunderLookup();
      ...
      // プログラム情報を設定（JPCOAR 2.0）
      let streamEntries = [];
      let streamId = '';
      let streamIdType = '';
      if (result.fundingStreams?.length) {
        // JGN 結果
        streamEntries = result.fundingStreams;
        streamId = result.fundingStreamId || '';
        streamIdType = streamId ? 'JGN_fundingStream' : '';
      } else if (isKakenhi(awardNum)) {
        // KAKENHI
        streamEntries = KAKENHI_FUNDING_STREAM;
      }

      // プログラム情報識別子フィールドを設定
      const fsIdInput = fundContent.querySelector('[data-field-key="subitem_funding_stream_identifier"] input');
      const fsIdTypeSelect = fundContent.querySelector('[data-field-key="subitem_funding_stream_identifier_type"] select');
      if (fsIdInput) fsIdInput.value = streamId;
      if (fsIdTypeSelect) fsIdTypeSelect.value = streamIdType;

      // プログラム情報エントリを設定
      if (streamEntries.length) {
        [...fsCont.querySelectorAll(':scope > .entry-group')].forEach(g => g.remove());
        streamEntries.forEach(fs => {
          const name = fs.fundingStream || fs.subitem_funding_stream || '';
          const lang = fs.fundingStreamLang ?? fs.subitem_funding_stream_language ?? '';
          const { grp, delBtn } = createEntryGroup();
          grp.appendChild(createFieldRow('プログラム情報', name, 'text', null, { fieldKey: 'subitem_funding_stream' }));
          grp.appendChild(createFieldRow('言語', lang, 'select', 'language', { fieldKey: 'subitem_funding_stream_language' }));
          grp.appendChild(delBtn);
          fsCont.appendChild(grp);
        });
      }
...
function renderOneRightsHolder(rh, idx, defLabel) {
  ...
  const { wrapper: rnWrap, content: rnCont } = createNestedSectionHeader('権利者名', 2, (cont) => {
    ...
    grp.appendChild(delBtn);
    cont.appendChild(grp);
  }, JPCOAR_SUBFIELD_LINKS.rightsholder_name);
  ...
  const { wrapper: riWrap, content: riCont } = createNestedSectionHeader('権利者識別子', 2, (cont) => {
    ...
    grp.appendChild(delBtn);
    cont.appendChild(grp);
  }, JPCOAR_SUBFIELD_LINKS.rightsholder_id);
...
// ----- ファイルパス抽出（webkitRelativePath から data/ 以降の相対パスを返す）-----
function extractFilePath(webkitRelativePath) {
  if (!webkitRelativePath) return null;
  const m = webkitRelativePath.match(/(^|\/)data\/(.+)$/);
  if (m) return m[2];
  return null; // data/ が見つからない
}
...
function renderOneFile(file, idx, defLabel) {
  ...
  fileInput.addEventListener('change', () => {
    ...
    // file_path 自動設定（単一ファイル選択時はファイル名のみ）
    setVal('file_path', f.name);
    // ヘッダーラベル更新
  ...
  fileCont.appendChild(createFieldRow('ファイルパス', file.file_path || '', 'text', null, { fieldKey: 'file_path' }));
...
function renderFileField(def, files) {
  ...
  // ----- data/ 以下のフォルダから一括追加 -----
  const folderPickRow = document.createElement('div');
  folderPickRow.className = 'field-row';
  folderPickRow.style.marginBottom = '4px';
  const folderLabel = document.createElement('span');
  folderLabel.className = 'field-label';
  folderLabel.textContent = 'data/\u4EE5\u4E0B\u306E\u30D5\u30A9\u30EB\u30C0\u3092\u9078\u629E';
  folderPickRow.appendChild(folderLabel);
  const folderBtn = document.createElement('button');
  folderBtn.type = 'button';
  folderBtn.className = 'btn';
  folderBtn.textContent = '\u30D5\u30A9\u30EB\u30C0\u3092\u9078\u629E\u2026';
  folderBtn.style.flex = '0 0 auto';
  folderPickRow.appendChild(folderBtn);
  content.appendChild(folderPickRow);

  const folderNote = document.createElement('div');
  folderNote.style.cssText = 'color:#666;margin:0 0 4px;font-size:0.85em;';
  folderNote.textContent = 'data/ \u307E\u305F\u306F data/ \u4EE5\u4E0B\u306E\u30D5\u30A9\u30EB\u30C0\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044\u3002\u30D5\u30A9\u30EB\u30C0\u76F4\u4E0B\u306EPDF\u30FB\u753B\u50CF\u30D5\u30A1\u30A4\u30EB\u304C\u8FFD\u52A0\u3055\u308C\u307E\u3059\u3002';
  content.appendChild(folderNote);

  const folderWarn = document.createElement('div');
  folderWarn.style.cssText = 'color:#c00;margin:0 0 8px;padding:4px 8px;display:none;font-size:0.9em;background:#fff0f0;border:1px solid #fcc;border-radius:4px;';
  content.appendChild(folderWarn);

  folderBtn.addEventListener('click', async () => {
    if (typeof window.showDirectoryPicker !== 'function') {
      folderWarn.textContent = '\u26A0 \u3053\u306E\u30D6\u30E9\u30A6\u30B6\u3067\u306F\u30D5\u30A9\u30EB\u30C0\u9078\u629E\u304C\u5229\u7528\u3067\u304D\u307E\u305B\u3093\u3002\u500B\u5225\u306E\u30D5\u30A1\u30A4\u30EB\u9078\u629E\u3092\u3054\u5229\u7528\u304F\u3060\u3055\u3044\u3002';
      folderWarn.style.display = '';
      return;
    }
    let dirHandle;
    try {
      ...
      folderWarn.textContent = '\u26A0 \u30D5\u30A9\u30EB\u30C0\u306E\u9078\u629E\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ' + e.message;
      folderWarn.style.display = '';
      return;
    }
    folderWarn.style.display = 'none';
    const selectedFolder = dirHandle.name;
    const isDataFolder = (selectedFolder === 'data');

    // フォルダ直下のファイルのみ列挙（サブフォルダは再帰しない）
    const directFiles = [];
    const IMG_EXTS = ['jpg','jpeg','png','gif','tiff','tif','bmp','webp','svg'];
    for await (const entry of dirHandle.values()) {
      if (entry.kind !== 'file') continue;
      const n = entry.name;
      if (n.startsWith('.')) continue;
      if (['desktop.ini', 'Thumbs.db'].includes(n)) continue;
      const ext = n.split('.').pop().toLowerCase();
      if (ext === 'pdf' || IMG_EXTS.includes(ext)) {
        directFiles.push(await entry.getFile());
      }
    }

    if (!directFiles.length) {
      folderWarn.textContent = '\u26A0 \u9078\u629E\u3055\u308C\u305F\u30D5\u30A9\u30EB\u30C0\u306E\u76F4\u4E0B\u306BPDF\u30FB\u753B\u50CF\u30D5\u30A1\u30A4\u30EB\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F\u3002';
      folderWarn.style.display = '';
      return;
    }

    const entries = directFiles.map(f => {
      const filePath = isDataFolder ? f.name : (selectedFolder + '/' + f.name);
      const type = f.type || '';
      const isImage = type.startsWith('image/') ||
        IMG_EXTS.includes(f.name.split('.').pop().toLowerCase());
      return {
        filename: f.name, format: type || 'application/octet-stream',
        ...
        url: { url: '', label: '', objectType: isImage ? 'thumbnail' : 'fulltext' },
        groups: '', version: '', file_path: filePath,
      };
    });

    // 追加ボタン（content.lastChild）の直前に挿入
    const addBtn = content.lastChild;
    for (const entry of entries) {
      const idx = content.querySelectorAll(':scope > .nested-item').length;
      content.insertBefore(renderOneFile(entry, idx, def.label), addBtn);
    }
  });
...
function updateBatchPanel() {
  ...
  panel.style.display = '';
  countEl.textContent = allMetadata.length;

  let html = '';
  allMetadata.forEach((m, i) => {
    const doi = m.system?.doi || '(DOI未設定)';
    const title = (m.item_30002_title0?.[0]?.subitem_title || '').substring(0, 60) || '(タイトル未設定)';
    const isCurrent = (i === currentBatchIndex);
    html += '<div class="batch-item' + (isCurrent ? ' batch-current' : '') + '" data-index="' + i + '">'
      + '<span class="batch-idx">' + (i + 1) + '.</span>'
      + '<span class="batch-doi">' + escHtml(doi) + '</span>'
      + '<span class="batch-title" title="' + escHtml(title) + '">' + escHtml(title) + '</span>'
      + '<button class="batch-rm" data-batch-remove="' + i + '" title="削除">✕</button>'
      + '</div>';
  });
...
function updateBatchNav() {
  ...
  if (!nav) return;
  if (allMetadata.length === 0 || currentBatchIndex < 0) {
    nav.style.display = 'none';
    return;
  }
  nav.style.display = 'flex';
  const m = allMetadata[currentBatchIndex];
  const doi = m?.system?.doi || '';
  document.getElementById('batch-nav-label').textContent =
    '編集中: ' + (currentBatchIndex + 1) + '/' + allMetadata.length + '件';
  document.getElementById('batch-nav-doi').textContent = doi;
  document.getElementById('batch-nav-prev').disabled = (currentBatchIndex <= 0);
  document.getElementById('batch-nav-next').disabled = (currentBatchIndex >= allMetadata.length - 1);
}

function removeBatchItem(index) {
  if (index < 0 || index >= allMetadata.length) return;
  const wasCurrent = (index === currentBatchIndex);
  allMetadata.splice(index, 1);
  // インデックス補正
  if (currentBatchIndex >= allMetadata.length) {
    currentBatchIndex = allMetadata.length - 1;
...
function clearBatch() {
  ...
  if (!confirm('蓄積された全' + allMetadata.length + '件のアイテムをクリアしますか？')) return;
  allMetadata = [];
  currentBatchIndex = -1;
  updateBatchPanel();
  document.getElementById('metadata-fields').innerHTML = '';
...
function navigateBatch(delta) {
  ...
  if (newIdx < 0 || newIdx >= allMetadata.length || newIdx === currentBatchIndex) return;
  saveCurrent();
...
// ===== 5.6a バッチ列展開用メタデータ統合 =====
// 複数metadataから列展開に必要な「最大サイズ」の仮想metadataを構築する。
// buildTsvColumnDefs() は配列フィールドの .length で展開列数を決定するため、
// 全metadataの最大長を持つ仮想オブジェクトを渡す。
function buildMaxSizeMetadata(metadataArray) {
  if (metadataArray.length === 0) return buildEmptyMetadata();
  if (metadataArray.length === 1) return metadataArray[0];

  const merged = { system: metadataArray[0].system };
  ...
  for (const key of allKeys) {
    ...
    const isArr = metadataArray.some(m => Array.isArray(m[key]));
    if (isArr) {
      // 配列: 最大長に合わせてダミーで埋める（buildTsvColumnDefs は length しか参照しない）
      const maxLen = Math.max(...metadataArray.map(m => Array.isArray(m[key]) ? m[key].length : 0));
      merged[key] = new Array(maxLen).fill({});
    } else {
      // objectフィールド: いずれかのmetadataで非空なら列を出力
      merged[key] = metadataArray.find(m => m[key] && !isTsvFieldEmpty(m[key]))?.[key]
                    || metadataArray[0]?.[key] || null;
    }
  }
  return merged;
}
  ...
  content.querySelectorAll(':scope > .nested-item.level-1').forEach(fundEl => {
    ...
    const funder = {
      ...
      subitem_funder_identifiers: {
        ...
        subitem_funder_identifier_type_uri: getFieldVal(fc, 'subitem_funder_identifier_type_uri'),
      },
      subitem_award_numbers: {
      ...
      subitem_award_titles: [],
      subitem_funding_stream_identifiers: {
        subitem_funding_stream_identifier: getFieldVal(fc, 'subitem_funding_stream_identifier'),
        subitem_funding_stream_identifier_type: getFieldVal(fc, 'subitem_funding_stream_identifier_type'),
        subitem_funding_stream_identifier_type_uri: getFieldVal(fc, 'subitem_funding_stream_identifier_type_uri'),
      },
      subitem_funding_streams: [],
    };
    fc.querySelectorAll('.entry-group').forEach(grp => {
      ...
      if (grp.querySelector('[data-field-key="subitem_funding_stream"]')) {
        funder.subitem_funding_streams.push({
          subitem_funding_stream: getFieldVal(grp, 'subitem_funding_stream'),
          subitem_funding_stream_language: getFieldVal(grp, 'subitem_funding_stream_language'),
        });
      }
    });
    items.push(funder);
...
function collectFileField(section) {
  ...
  content.querySelectorAll(':scope > .nested-item.level-1').forEach(fileEl => {
    ...
    const file = {
      ...
      file_path: getFieldVal(fc, 'file_path'),
...
// ================================================================
// STEP 8a: TSV エクスポート（tsv_headers.json テンプレート駆動）
// ================================================================
...
// ユーザが貼り付けたTSVヘッダーテキスト（5行ヘッダー）を完全パースし、テンプレート配列を返す。
// 2行以上で先頭セルが '#ItemType' を含む場合に完全パース。
// 不足行は TSV_HEADERS_TEMPLATE から補完。パースできない場合は null を返す。
function parseCustomTemplate(templateText) {
  ...
  // 先頭行が ItemType 行かどうか判定
  const firstCell = (lines[0][0] || '').trim();
  if (!firstCell.includes('#ItemType') && !firstCell.includes('ItemType')) return null;

  // row0: ItemType行、row1: プロパティキー行は必須
  const row0 = lines[0];
  const row1 = lines[1];
  const colCount = row1.length;

  // 不足行はデフォルトから補完（pad して列数を揃える）
  const pad = (row) => {
    const r = row || [];
    return r.length >= colCount ? r.slice(0, colCount) : [...r, ...Array(colCount - r.length).fill('')];
  };

  const row2 = lines.length > 2 ? pad(lines[2]) : pad(TSV_HEADERS_TEMPLATE[2]);
  const row3 = lines.length > 3 ? pad(lines[3]) : pad(TSV_HEADERS_TEMPLATE[3]);
  const row4 = lines.length > 4 ? pad(lines[4]) : pad(TSV_HEADERS_TEMPLATE[4]);

  return [pad(row0), pad(row1), row2, row3, row4];
}

// ユーザが貼り付けたテキストまたはメタデータのキーからプロパティキープレフィックスを検出。
// 戻り値例: 'item_30002_' / 'item_40039_'
...
// テンプレートからフィールドグループを構築。
// 各グループは連続する列の集まりで、同一トップレベルフィールドキーを持つ。
// prefix が 'item_30002_' と異なる場合、出力キー（key）をその prefix に置換する。
// lookupKey は常に 'item_30002_' ベースで metadata アクセスに使う。
// template が指定されればそれを使用、なければ TSV_HEADERS_TEMPLATE を使用。
function groupTsvColumns(prefix, template) {
  const DEFAULT_PREFIX = 'item_30002_';
  const tmpl = template || TSV_HEADERS_TEMPLATE;
  const [, keyRow, labelRow, sysRow, conRow] = tmpl;

  // カスタムテンプレートのキー行で使われているプレフィックスを検出
  let tmplPrefix = DEFAULT_PREFIX;
  if (template) {
    for (const k of keyRow) {
      const m = (k || '').match(/\.metadata\.(item_\d+_)/);
      if (m) { tmplPrefix = m[1]; break; }
    }
  }
  ...
  for (let i = 0; i < keyRow.length; i++) {
    const rawKey = keyRow[i] || '';
    if (!rawKey) continue;
    const baseKey  = rawKey.replace(/^#/, '');        // '#.id' → '.id'

    // lookupKey: metadata アクセス用に常に item_30002_ ベースに正規化
    const lookupKey = tmplPrefix !== DEFAULT_PREFIX
      ? baseKey.replace(tmplPrefix, DEFAULT_PREFIX)
      : baseKey;

    const label    = (labelRow[i] || '').replace(/^#/, '');
    const sys      = (sysRow[i]   || '').replace(/^#$/, '');
    const con      = (conRow[i]   || '').replace(/^#$/, '');

    if (isTsvExcluded(lookupKey)) continue;

    // outKey: カスタムテンプレートではキーをそのまま使用、
    //         デフォルトテンプレートでは検出プレフィックスに置換
    const outKey = template
      ? baseKey
      : ((prefix !== DEFAULT_PREFIX) ? baseKey.replace(DEFAULT_PREFIX, prefix) : baseKey);

    // フィールドグループキー: 正規化済み lookupKey の '.metadata.item_XXXX' 部分
    let fieldKey;
    if (lookupKey.startsWith('.file_path')) {
    ...
    if (!cur || cur.fieldKey !== fieldKey) {
      cur = { fieldKey, isExpandable: false, cols: [] };
      groups.push(cur);
    }
    ...
    if (lookupKey.match(/^\.metadata\.item_[^.\[]+\[0\]/) || lookupKey.match(/^\.file_path\[0\]/)) cur.isExpandable = true;
    cur.cols.push({ key: outKey, lookupKey, label, sys, con });
  }
  return groups;
}

function isTsvFieldEmpty(value) {
...
// フィールドグループを metadata に基づいて展開し、最終列定義配列を返す。
// template が指定されればカスタムテンプレートを使用。
function buildTsvColumnDefs(prefix, metadata, template) {
  const groups = groupTsvColumns(prefix, template);
  const columns = [];

  for (const group of groups) {
    if (group.fieldKey === '__system__') {
      columns.push(...group.cols);
      continue;
    }
    const metaKey = group.fieldKey.replace(/^\.metadata\./, ''); // 'item_30002_creator2'

    if (!group.isExpandable) {
      if (isTsvFieldEmpty(metadata[metaKey])) continue;
      columns.push(...group.cols);
    } else {
      const arr = metadata[metaKey];
      const size = Array.isArray(arr) ? arr.length : 0;
      if (size === 0) continue;
      for (let i = 0; i < size; i++) {
        for (const col of group.cols) {
          // replace() は最初の '[0]' のみ置換 = トップレベル配列インデックスのみ展開。
          // 内部配列（creatorNames[0] 等）は WEKO3 TSV仕様上 [0] 固定のため意図的。
          columns.push({
            key:       col.key.replace('[0]', `[${i}]`),
            lookupKey: col.lookupKey.replace('[0]', `[${i}]`),
            label:     col.label.replace('[0]', `[${i}]`),
            sys:       col.sys,
            con:       col.con,
          });
        }
...
// → ['item_30002_creator2', 0, 'creatorNames', 0, 'creatorName']
function parseTsvPath(path) {
  return path.replace(/\[(\d+)\]/g, '.$1').split('.').filter(Boolean).map(p => {
...
const TSV_SYS_KEY_MAP = {
  ...
  '.feedback_mail[0]': 'feedback_mail', '.researchmap_linkage': 'researchmap_linkage', '.cnri': 'cnri',
  '.doi_ra': 'doi_ra', '.doi': 'doi', '.edit_mode': 'edit_mode',
  '.metadata.pubdate': 'pubdate',
};

function getTsvValue(col, metadata) {
  // file_path フィールド（data/ 以降の相対パス or ファイル名）
  ...
  if (fpMatch) {
    ...
    return files[idx]?.file_path || files[idx]?.filename || '';
  }
  // システムフィールド
  ...
  // メタデータフィールド: lookupKey（item_30002_ ベース）でナビゲート
...
function generateTsv(metadataArray, templateText, repoHost) {
  // 後方互換: 単一 metadata が渡された場合は配列にラップ
  if (!Array.isArray(metadataArray)) metadataArray = [metadataArray];
  if (metadataArray.length === 0) return null;

  // カスタムテンプレートのパース試行
  const customTemplate = parseCustomTemplate(templateText);
  const effectiveTemplate = customTemplate || TSV_HEADERS_TEMPLATE;

  // 列定義: 全metadataの最大サイズで展開
  const maxMeta = buildMaxSizeMetadata(metadataArray);
  const prefix = detectTsvPrefix(templateText, maxMeta);
  const cols = buildTsvColumnDefs(prefix, maxMeta, customTemplate);
  if (!cols.length) return null;

  // ItemType行: repoHost でスキーマURLを置換
  const tmplRow0 = effectiveTemplate[0];
  const n = cols.length;
  let schemaUrl = tmplRow0[2] || '';
  if (repoHost) {
    let host = repoHost.trim();
    if (host && !host.endsWith('/')) host += '/';
    if (host) schemaUrl = schemaUrl.replace('https://localhost/', host);
  }
  const row1 = [tmplRow0[0] || '#ItemType', tmplRow0[1] || '', schemaUrl, ...Array(Math.max(0, n - 3)).fill('')];
...
function exportTsv() {
  ...
  const items = allMetadata.length > 0 ? [...allMetadata] : [collectFromDOM()];
  // カスタムテンプレート: #122 対応方針決定まで無効化（デフォルトアイテムタイプ（フル）のみ使用）
  const templateText = '';
  const repoHost = document.getElementById('repo-host')?.value || '';
  const tsv = generateTsv(items, templateText, repoHost);
  if (!tsv) { alert('TSV出力できるデータがありません。'); return; }

  let filename;
  if (items.length > 1) {
    // 複数件: タイムスタンプベースのファイル名
    const now = new Date();
    const pad = n => String(n).padStart(2, '0');
    const ts = now.getFullYear() + pad(now.getMonth() + 1) + pad(now.getDate())
      + '_' + pad(now.getHours()) + pad(now.getMinutes()) + pad(now.getSeconds());
    filename = `import_${ts}.tsv`;
  } else {
    // 1件: 既存のDOIベースファイル名
...
function buildFundingPreview(funders) {
  ...
  nonEmpty.forEach((f, i) => {
    ...
    const fsIdObj = f.subitem_funding_stream_identifiers || {};
    const fsIdStr = fsIdObj.subitem_funding_stream_identifier
...
function buildFilePreview(files) {
  ...
  let html = '<table class="pv-inner-table"><thead><tr>';
  html += '<th>#</th><th>\u30D5\u30A1\u30A4\u30EB\u540D</th><th>\u30D1\u30B9</th><th>\u30B5\u30A4\u30BA</th><th>\u5F62\u5F0F</th><th>\u30A2\u30AF\u30BB\u30B9</th><th>\u30E9\u30A4\u30BB\u30F3\u30B9</th>';
  html += '</tr></thead><tbody>';

  nonEmpty.forEach((f, i) => {
    const size = f.filesize?.[0]?.value || '';
    const license = LICENSE_TYPE_LABELS[f.licensetype] || f.licensetype || '';
    html += '<tr><td>' + i + '</td><td>' + fmtVal(f.filename) + '</td><td>' + fmtVal(f.file_path || '') + '</td><td>' + (size || '') + '</td><td>' + (f.format || '') + '</td><td>' + (f.accessrole || '') + '</td><td>' + license + '</td></tr>';
  });
  html += '</tbody></table>';
  return html;
}
...
(async function init() {
  ...
  // 「ページからDOI取得」はChrome拡張版（panel.html）のみに存在するボタン
  const getDoiBtn = document.getElementById('get-doi-from-page');
  if (getDoiBtn) getDoiBtn.addEventListener('click', async (e) => {
  ...
  // バッチナビゲーション
  document.getElementById('batch-nav-prev').addEventListener('click', function() { navigateBatch(-1); });
  document.getElementById('batch-nav-next').addEventListener('click', function() { navigateBatch(1); });

  // バッチ管理パネル: イベント委譲（MV3 CSP対応）
  ...
  document.getElementById('batch-list').addEventListener('click', function(e) {
    // 削除ボタン
    const rmIdx = e.target.dataset?.batchRemove;
    if (rmIdx != null) { removeBatchItem(parseInt(rmIdx, 10)); return; }
    // アイテムクリック → 表示切替
    const item = e.target.closest('.batch-item');
    if (item) {
      const idx = parseInt(item.dataset.index, 10);
      if (!isNaN(idx) && idx >= 0 && idx < allMetadata.length && idx !== currentBatchIndex) {
        saveCurrent();
