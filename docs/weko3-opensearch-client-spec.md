...
「入力をクリア」は検索条件、タイトル完全一致、ID種別、並べ替え、詳細検索の開閉状態とエラー表示だけを初期化する。リポジトリURL、検索済み結果、stateは維持するため、クリア後のページ移動も初回検索条件を使用する。検索中はクリアできない。
...
- Chrome拡張では結果の外部リンクを現在のアクティブタブで開き、サイドパネルの検索状態を保持する。遷移できない特殊タブでは背面タブへフォールバックする。
...
### `DOI` と `selfDOI` の追加実測（2026-07-17、#241）

OpenAlex機関別著作検索の登録済み照合へ識別子検索を追加するため、`id_attr=DOI` と `id_attr=selfDOI` を3機関で比較した。

返戻件数だけでは一致を判定せず、JPCOAR内の `identifier`、`relatedIdentifier`、`identifierRegistration` を正規化して候補DOIとの完全一致を確認した。

| 機関 | 候補DOI | `id_attr=DOI` | `id_attr=selfDOI` |
|---|---|---|---|
| 筑波大学 | `10.2964/jsik_2016_001` | 1件、完全一致あり | 0件 |
| JIRCAS | `10.18919/jkg.75.2_52` | 1件、完全一致あり | 0件 |
| JIRCAS | `10.34556/0002000556` | 20件、完全一致なし | 5件、完全一致あり |
| JIRCAS | `10.34556/0002000577` | 0件 | 1件、完全一致あり |
| JIRCAS | `10.34556/0002000483` | 0件 | 1件、完全一致あり |
| 国立国語研究所 | `10.15084/00001012` | 0件 | 1件、完全一致あり |
| 国立国語研究所 | `10.15084/00000832` | 0件 | 1件、完全一致あり |
| 国立国語研究所 | `10.15084/00001075` | 0件 | 1件、完全一致あり |

実測した範囲では、外部DOIを `DOI`、自リポジトリが登録したDOIを `selfDOI` で検索できた。

たとえば、JIRCASのOAI-PMH出力に `<jpcoar:identifierRegistration identifierType="JaLC">10.34556/0002000483</jpcoar:identifierRegistration>` が存在しても、`id_attr=DOI` は0件となり、`id_attr=selfDOI` で対象レコードを取得できた。

この結果は、JPCOAR出力にDOIが存在するだけでは `id_attr=DOI` の検索対象になるとは限らないことを示す。

登録済み照合では `DOI`、`selfDOI` の順に検索し、どちらも正常に完全一致しない場合だけタイトル検索へ進む。

通信エラー、HTTPエラー、XML解析エラーが起きた場合は後続検索へ進まず、照合不可として扱う。

今回の範囲では、同一DOIに完全一致するレコードが複数返るケースは確認できなかった。
...
- 件数ラベルの開始位置はAPIの `startIndex` に依存せず、現在ページと20件固定のページサイズから算出する。
- ページング中の通信エラーでは直前ページの結果とページャを維持する。
- 「入力をクリア」で検索条件だけが初期化され、リポジトリURL・結果・ページングstateは維持される。
