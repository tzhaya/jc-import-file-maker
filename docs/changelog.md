...
| 日付 | 内容 |
|------|------|
| 2026-06-28 | 設定（APIキー・初期値）を別ページに集約（[#181](https://github.com/tzhaya/jc-import-file-maker/issues/181)）：`docs/settings.md` 新規作成し全8項目（APIキー3＋初期値5）を一元化（初期値表に欠落していた `DEFAULT_ROR_ID`・`DEFAULT_OPENALEX_DAYS` を追加、`DEFAULT_REPOSITORY_URL` の反映先にOpenAlex機関検索の登録状況照合(#156)を明記）。README の設定セクションを要約＋リンクに縮約、`docs/user_guide.md` の設定関連記述を `settings.md` への導線に整理、README/`developer_docs.md`/`index.html` のドキュメント一覧に追記、`scripts/check.js` のUTF-8チェック対象に追加 |
