...
最終更新: 2026-07-17（ドキュメントインデックスとロードマップの整備）

## ドキュメントインデックスとロードマップの整備（2026-07-17）

`docs/README.md` を新設し、`docs/` 以下の文書を目的別に一覧化した。現行ファイルへのリンクに加え、将来的なフォルダ構成案と、GitHub Pagesの公開URLを考慮して移設する必要がある文書を記載した。`roadmap_2026-07-05.md` は、少数実機での取り込み成功と検証完了を区別し、OA Assistとの役割分担と今後の優先順位が一方向に読めるよう推敲した。両文書を開発者向け一覧とUTF-8検査対象へ追加した。ドキュメントのみの変更のため、テスト用HTML同期、E2Eテスト、`LOCAL_VERSION`、Chrome拡張manifestの更新は対象外。
...
README、使い方ガイド、機能と技術の説明は情報を網羅していたが、長い機能列挙が続き、利用者が「何を選び、どこから始めるか」を判断しにくい構成になっていた。
機能事実や操作手順は変えず、導入、段落、列挙の順序を見直した。

### 修正内容

1. `README.md`の冒頭に、ツールが減らす作業と通常ブラウザ版、Chrome拡張機能版の選択基準を追加。
2. READMEの導入手順と基本操作を整理し、「入植して取得」の誤字を修正。
3. 助成情報検索、OpenSearch検索、OpenAlex機関別著作検索を、機能一覧ではなく利用場面から説明する構成へ変更。
4. `docs/user_guide.md`の冒頭、データ補完、識別子逆引き、バッチ処理、自動保存、検索制約、DataCite制限事項の導入を整理。
5. `function.md`に各Phaseの役割を示す導入を追加し、機能追加の流れを追いやすくした。

### 検証

- 文書のみの変更のため、テスト用HTML同期、E2Eテスト、Chrome拡張への反映、`LOCAL_VERSION`、manifest更新は対象外。
- `npm test`で品質チェックを実施。

## WEKO3 OpenSearch検索項目拡充（2026-07-14・#237）

### 概要

Chrome拡張のリポジトリコンテンツ検索に、標準・詳細検索、ID検索、作成日時ソートを追加した。資源タイプを原典準拠の全78件へ更新し、JPCOARレスポンスのページ内逆順を補正した。`version`と日付範囲検索は実環境での対応差が大きいため対象外とした。

### 実装

1. DOM読取、`normalizeQuery()`、`validateQuery()`、`buildUrl()`を分離し、許可パラメータだけを送信。
2. 新規検索を`startSearch()`、ページ移動を`loadPage()`へ分け、リポジトリorigin・query・sortをstateに保存。
3. ID値とID種別の片側欠落を通信前に検出し、不足欄へフォーカス。
4. `normalizeJpcoarItemOrder()`でページ内順序を非破壊に補正。
5. `tests/opensearch.test.js`で語彙78件、入力正規化、ID検証、URL、対象外パラメータ、順序補正を検証。
6. OpenSearch仕様書、要件、利用ガイド、開発者文書、README、変更履歴を更新。
7. 実機確認で各検索がHTTP 406となったため、`format=jpcoar` と競合する独自Acceptヘッダーを削除。WEKO3にはブラウザ既定ヘッダーで送信する。
8. 実機確認に基づき、言語検索のクエリパラメータを `language` ではなく `lang` に修正。
9. PRレビューで確認された既存不具合を同梱修正。結果リンクは現在のアクティブタブで開いてサイドパネル状態を保持し、件数ラベルはページ番号から算出。ページング通信失敗時は直前ページの結果とページャを維持する。
10. 「入力をクリア」で検索条件・完全一致指定・ID種別・並べ替えを初期化。リポジトリURL・検索済み結果・ページングstateは維持し、検索中は操作不可とした。

### 実機確認項目

- JIRCAS・筑波大学で標準／詳細検索、ID検索、wid、`conference output`、作成日時昇降順を確認。
- 2ページ目へ移動後にフォーム・URL・sortを変更し、次ページと1ページ目へ戻る操作で初回条件が保持されることを確認。
- 詳細表示、ファイルリンク、エラー、コンソール未処理例外を回帰確認。

2026-07-14、ユーザーがChrome拡張実機で検索を確認。当初は独自AcceptヘッダーによりHTTP 406となったため削除し、再確認で検索可能となった。言語検索は実測に基づき `language` から `lang` へ修正し、正常動作を確認。その他の手動確認項目も問題なし。
