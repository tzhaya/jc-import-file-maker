...
- 1ページ20件で表示します。「前へ」「次へ」で移動しても、最初に検索したリポジトリURL・検索条件・並べ替えを維持します。フォームを変更した内容を反映するには、もう一度「検索」を押してください。
- レコードの作成日時は昇順・降順に並べ替えられますが、作成日時の範囲検索はできません。
- `version`（バージョン種別）、Issued / Available等の日付範囲、MIMEタイプ、WEKOアイテムタイプ、その他の詳細パラメータは今回の検索画面では対応していません。
- OpenSearchは公開済みアイテムだけを返します。非公開・ワークフロー処理中のアイテムは検索結果に含まれません。
