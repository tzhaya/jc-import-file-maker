...
DOIから書誌情報を取得できても、そのままJAIRO Cloudへ登録できるとは限りません。
メタデータの確認、JPCOARスキーマへの対応、TSV形式への変換が必要です。

このツールは、Crossref、OpenAlex、JaLCなどのAPIからメタデータを取得し、JPCOARスキーマに準拠した形式で確認、編集して、JAIRO Cloud（WEKO3）インポート用TSVを生成します。

通常ブラウザ版は、`make_jc_importer.html`、`shared.js`、`tsv_headers_template.js`を同じフォルダに置き、`make_jc_importer.html`をブラウザで開くだけで使用できます。
インストールは不要です。
サイドパネルで利用できるChrome拡張機能版もあります（導入方法は[README](../README.md#導入方法)を参照）。
...
DOIから得られる情報だけでは、所属機関、助成情報、オープンアクセス状態などが不足することがあります。
そのため、Crossref、DataCite、JaLCの情報を基礎に、OpenAlex、ROR、CiNii Research、KAKEN XML、JGN（Japan Grant Number）、Open Policy Finder（OPF）の情報で補完します。
...
機関名は表記ゆれが生じやすいため、名称だけでは同じ機関か判断しにくいことがあります。
所属機関や助成機関のフィールドでは、ROR IDなどの識別子から機関名を逆引きできます。
科研費課題番号やJGN課題番号から、助成機関名と助成機関識別子を補完することもできます。
...
DOIを追加で取得しても、先に編集した内容は失われません。
複数の論文をバッチに蓄積し、1つのTSVファイルへまとめて出力できます。
...
長いメタデータを編集中に、タブやサイドパネルを閉じてしまうことがあります。
入力中、編集中のメタデータ（バッチに蓄積したアイテムを含む）は、**自動的にブラウザへ保存**されるため、次回起動時に作業内容を復元できます。
...
サイドパネルの「リポジトリコンテンツ検索」タブでは、JAIRO Cloud / WEKO3の公開済みアイテムを検索できます。

### 検索する

1. **リポジトリURL**に検索対象機関のURLを入力します。設定ページでデフォルトリポジトリURLを登録している場合は自動入力されます。
2. 標準検索から1項目以上を入力します。
   - キーワード
   - タイトル（必要に応じて「タイトル完全一致」を選択）
   - 作成者・著者
   - 内容記述
   - 資源タイプ（全78件の語彙から選択）
   - ID値＋ID種別
3. 必要に応じて「**詳細検索**」を開き、主題・出版者・言語・収録物名・作成者名識別子を入力します。
4. 並べ替えを、既定順・作成日時が古い順・作成日時が新しい順から選択します。
5. 「**検索**」を押すと、タイトル・作成者・書誌情報・ファイルリンク等が一覧表示されます。タイトルをクリックするとJPCOARメタデータの詳細を確認できます。

「**入力をクリア**」を押すと、検索条件、タイトル完全一致、ID種別、並べ替えを初期状態へ戻せます。リポジトリURLと表示済みの検索結果は残ります。新しい条件で検索するには、入力後に「検索」を押してください。

ID検索ではID値とID種別を必ず両方指定してください。片方だけでは検索を実行しません。DOI、NCID等の検索可否や、作成者名識別子として利用できるORCID・WEKO著者ID等は、リポジトリの設定によって異なります。検索結果が1件だけになることや完全一致は保証されないため、対象アイテムが結果に含まれるか確認してください。

### ページングと制約

検索結果は1ページ20件です。
「前へ」「次へ」で移動しても、検索を実行した時点のリポジトリURL、検索条件、並べ替えを維持します。
フォームの変更を反映するには、もう一度「検索」を押してください。

レコードの作成日時は昇順または降順に並べ替えられますが、作成日時の範囲検索はできません。
`version`（バージョン種別）、IssuedやAvailableなどの日付範囲、MIMEタイプ、WEKOアイテムタイプ、その他の詳細パラメータにも対応していません。

また、OpenSearchが返すのは公開済みアイテムだけです。
非公開またはワークフロー処理中のアイテムは検索結果に含まれません。
...
「**リポジトリURL**」欄に自機関リポジトリのURLを入力すると、各候補の行に登録状況バッジが付きます。まず `DOI`、次に `selfDOI` の識別子種別で検索し、返戻レコード内で候補DOIが完全一致すれば登録済みの可能性大と判定します。識別子検索は機関・インデックス設定に依存するため、両方が正常に0件またはDOI不一致となった場合は、**候補タイトルで検索して返戻レコードのDOIと突き合わせる従来方式へフォールバックします**（OpenSearch は CORS 制約のため、この照合は拡張機能版でのみ動作します。通常ブラウザ版では照合列は表示されません）。

判定の流れ:

1. 候補DOIを `id=<DOI>&id_attr=DOI` で検索し、返戻JPCOAR内の識別子を完全一致で確認します。一致すればここで「登録済みの可能性大」と確定します。
2. `DOI` 検索が正常に0件またはDOI不一致なら、`id_attr=selfDOI` で同じ候補DOIを検索します。完全一致すれば「登録済みの可能性大」と確定します。
3. `DOI` と `selfDOI` の両方が正常に0件またはDOI不一致なら、候補論文のタイトルを正規化します（HTMLタグ片を除去し、先頭の十数語に切り詰め。表記ゆれや特殊文字での取りこぼしを避け、再現率を優先します）。
4. そのタイトルで自機関リポジトリの OpenSearch を検索します。
5. 返ってきた各レコードの JPCOAR メタデータ内の識別子（`jpcoar:identifier`・関連情報の `relatedIdentifier`・`identifierRegistration`）からDOIを抽出します。
6. 候補のDOIと抽出したDOIを、両方とも小文字化・`https://doi.org/` 等のプレフィックス除去で正規化して突き合わせ、次の3値で判定します。

| バッジ | 条件 | 既定のチェック | 意味 |
|--------|------|--------------|------|
| ⚪ 登録済みの可能性大 | DOI ID検索、またはタイトル検索後の照合でDOIが一致 | OFF | 同じDOIのレコードが既にリポジトリにある可能性が高い |
...
- DOI ID検索・タイトル検索・DOI照合はいずれも機関設定やメタデータ品質の影響を受け、誤ヒット・取りこぼしが残ります。バッジは判断の補助であり、**最終判断はユーザーが行ってください**。
- DOI ID検索の通信・HTTP・XML解析エラーはタイトル検索へ進まず「⚠ 照合不可」と表示し、既定チェックをOFFにします。障害を「未登録の可能性」と誤表示しないための安全側の動作です。
- 候補1件につき OpenSearch クエリを最大3回（`DOI` → `selfDOI` → タイトル）、候補間は間隔をあけて順次実行します（自機関リポジトリへの負荷に配慮）。件数が多いと照合に時間がかかります。
...
DataCite DOIからもメタデータを取得できますが、CrossrefやJaLCと同じ項目がそろうわけではありません。
メタデータ構造の違いにより、次の項目は自動取得または自動判定に制限があります。
取得後に内容を確認し、必要な項目を手動で補完してください。
