...
「**リポジトリURL**」欄に自機関リポジトリのURLを入力すると、各候補の行に登録状況バッジが付きます。WEKO3 OpenSearchにはID検索もありますが、DOI等の対応状況は機関・識別子種別により異なります。この登録状況判定では互換性を優先し、**候補のタイトルで検索して、返ってきたレコードのDOIと突き合わせます**（OpenSearch は CORS 制約のため、この照合は拡張機能版でのみ動作します。通常ブラウザ版では照合列は表示されません）。
