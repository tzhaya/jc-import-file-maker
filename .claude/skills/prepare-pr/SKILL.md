...
一項目でも未完了ならPR作成へ進まない。**コミットではなくPR作成を促す。**

正準実装（リポジトリ level）を呼ぶ薄い層とする:
- テスト要約: `npm test`（= `scripts/run-tests.js`）
- E2E記録の突合: `scripts/verify-e2e-record.mjs`
- E2E対象・記録形式の共通設定: `scripts/e2e-config.mjs`
...
準備後、対象に応じてユーザーへ`/e2e-test`の手動実行を依頼する。Claudeから起動しない。
