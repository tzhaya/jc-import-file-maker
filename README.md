...
| ツール | 日付 | バージョン | 更新概要 |
|--------|------|-----------|----------|
| Chrome拡張機能版 | 2026-07-09 | ver. 1.21.2 | DataCite APIが404を返す場合のエラーメッセージを改善（[#211](https://github.com/tzhaya/jc-import-file-maker/issues/211)）。DOI Registration AgencyがDataCiteと判定されてもDataCite API側でメタデータを取得できない（削除・非公開・Gone状態等の）DOIがあるため、原因の可能性がわかる文言に変更 |
| インポート用TSV生成ツール | 2026-07-09 | — | DataCite APIが404を返す場合のエラーメッセージを改善（[#211](https://github.com/tzhaya/jc-import-file-maker/issues/211)）。DOI Registration AgencyがDataCiteと判定されてもDataCite API側でメタデータを取得できない（削除・非公開・Gone状態等の）DOIがあるため、原因の可能性がわかる文言に変更 |
