...
| ツール | 日付 | バージョン | 更新概要 |
|--------|------|-----------|----------|
| Chrome拡張機能版 | 2026-07-08 | ver. 1.20.0 | Crossrefパスの本文言語を `crJson.language`（ISO 639-1、地域タグ対応）から639-2/Tへ変換して設定するよう修正（[#223](https://github.com/tzhaya/jc-import-file-maker/issues/223)）。従来は無条件で `eng` 固定だった。変換不能・language未提供時は `eng`＋「要確認」バッジを表示 |
| インポート用TSV生成ツール | 2026-07-08 | — | Crossrefパスの本文言語を `crJson.language`（ISO 639-1、地域タグ対応）から639-2/Tへ変換して設定するよう修正（[#223](https://github.com/tzhaya/jc-import-file-maker/issues/223)）。従来は無条件で `eng` 固定だった。変換不能・language未提供時は `eng`＋「要確認」バッジを表示 |
