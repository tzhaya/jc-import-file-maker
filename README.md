...
| ツール | 日付 | バージョン | 更新概要 |
|--------|------|-----------|----------|
| Chrome拡張機能版 | 2026-07-08 | ver. 1.21.1 | `calcEmbargoEndDate()` のタイムゾーン依存バグを修正（PR #233 コードレビュー指摘）。UTCパースとローカル時刻メソッドの混在により、UTCより遅れたタイムゾーンで実行するとエンバーゴ満了日が1日ずれ、アクセス権判定に影響し得た不具合をUTC系メソッド統一で解消 |
| インポート用TSV生成ツール | 2026-07-08 | — | `calcEmbargoEndDate()` のタイムゾーン依存バグを修正（PR #233 コードレビュー指摘）。UTCパースとローカル時刻メソッドの混在により、UTCより遅れたタイムゾーンで実行するとエンバーゴ満了日が1日ずれ、アクセス権判定に影響し得た不具合をUTC系メソッド統一で解消 |
