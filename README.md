...
| ツール | 日付 | バージョン | 更新概要 |
|--------|------|-----------|----------|
| Chrome拡張機能版 | 2026-07-08 | ver. 1.19.3 | サイドパネルで助成情報検索・OpenAlex機関別著作検索の結果URLをクリックするとパネルが「DOIインポート」に戻る不具合を再修正（[#228](https://github.com/tzhaya/jc-import-file-maker/issues/228)、#201の修正が不完全だった再発）。現在のアクティブタブ内でリンク先を開く方式に変更し、パネルの表示状態を保持 |
| インポート用TSV生成ツール | 2026-07-08 | — | バッチ全クリア/最後の1件削除時にDOM参照ミスでエラーになり下書きが復活する不具合（[#221](https://github.com/tzhaya/jc-import-file-maker/issues/221)）、助成情報検索後にアコーディオンのラベルが更新されない不具合（[#224](https://github.com/tzhaya/jc-import-file-maker/issues/224)）、TSVエクスポートのファイル名にDOI由来のWindows禁止文字が残る不具合（[#226](https://github.com/tzhaya/jc-import-file-maker/issues/226)）を修正 |
| 助成情報検索ツール | 2026-07-08 | — | サイドパネルで検索結果URLをクリックするとパネルが「DOIインポート」に戻る不具合を修正（[#228](https://github.com/tzhaya/jc-import-file-maker/issues/228)）。`parseTsvTemplate()` のHTML/拡張JS間の実装揺れを解消（[#227](https://github.com/tzhaya/jc-import-file-maker/issues/227)） |
