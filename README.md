...
| ツール | 日付 | バージョン | 更新概要 |
|--------|------|-----------|----------|
| Chrome拡張機能版 | 2026-07-08 | ver. 1.21.0 | アクセス権判定にエンバーゴ満了日の判定を追加（[#225](https://github.com/tzhaya/jc-import-file-maker/issues/225)）。発行日とエンバーゴ期間から公開可能日を算出し、全エンバーゴが満了済みなら `open access` を設定するよう変更（従来は1件でもエンバーゴがあれば無条件で `embargoed access`）。満了日不明・未満了時は従来どおり安全側 |
| インポート用TSV生成ツール | 2026-07-08 | — | アクセス権判定にエンバーゴ満了日の判定を追加（[#225](https://github.com/tzhaya/jc-import-file-maker/issues/225)）。発行日とエンバーゴ期間から公開可能日を算出し、全エンバーゴが満了済みなら `open access` を設定するよう変更（従来は1件でもエンバーゴがあれば無条件で `embargoed access`）。満了日不明・未満了時は従来どおり安全側 |
